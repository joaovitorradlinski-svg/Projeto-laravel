...
@section('content')
<form action="{{ route('locais.update', $local->id) }}" method="POST" enctype="multipart/form-data" class="max-w-lg mx-auto mt-8 bg-[#181a20] rounded shadow p-6">
    @csrf
    @method('PUT')
    ...
    <select name="campanha_id" id="campanha_id" class="mb-4 w-full px-3 py-2 bg-[#23272f] border border-gray-700 text-gray-100 rounded">
        @foreach($campanhas as $campanha)
            <option value="{{ $campanha->id }}" @if($local->campanha_id == $campanha->id) selected @endif>{{ $campanha->titulo }}</option>
        @endforeach
    </select>

    <label for="nome" class="block text-gray-300 mb-2">Nome</label>
    <input type="text" name="nome" id="nome" value="{{ $local->nome }}" class="w-full px-3 py-2 rounded bg-[#23272f] border border-gray-700 text-gray-100 mb-4"/>

    <label for="descricao" class="block text-gray-300 mb-2">Descrição</label>
    <textarea name="descricao" id="descricao" class="w-full px-3 py-2 rounded bg-[#23272f] border border-gray-700 text-gray-100 mb-4">{{ $local->descricao }}</textarea>
    ...
    @enderror
    @if($local->imagem)
        <img src="{{ asset('storage/' . $local->imagem) }}" alt="{{ $local->nome }}" class="mb-4 rounded w-32 h-32 object-cover border border-gray-800"/>
    @endif
